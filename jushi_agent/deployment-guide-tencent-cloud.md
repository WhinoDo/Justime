...
- 前端：`jushi_agent`（Next.js 14）
- 后端：`jushi_backend`（FastAPI）
...
```bash
sudo mkdir -p /opt/jushi
sudo chown -R $USER:$USER /opt/jushi
cd /opt/jushi
git clone <你的仓库地址> .
```
...
## 5.1 前端：`/opt/jushi/jushi_agent/.env.local`
...
## 5.2 后端：`/opt/jushi/jushi_backend/.env`
...
## 7.1 后端服务 `/etc/systemd/system/jushi-backend.service`

```ini
[Unit]
Description=Jushi FastAPI Backend
After=network.target

[Service]
User=ubuntu
WorkingDirectory=/opt/jushi/jushi_backend
EnvironmentFile=/opt/jushi/jushi_backend/.env
ExecStart=/opt/jushi/jushi_backend/.venv/bin/uvicorn app.main:app --host 127.0.0.1 --port 8080 --workers 2
Restart=always
RestartSec=5
...
## 7.2 前端服务 `/etc/systemd/system/jushi-agent.service`

```ini
[Unit]
Description=Jushi Next.js Frontend
After=network.target

[Service]
User=ubuntu
WorkingDirectory=/opt/jushi/jushi_agent
...
sudo systemctl enable --now jushi-backend
sudo systemctl enable --now jushi-agent
sudo systemctl status jushi-backend --no-pager
sudo systemctl status jushi-agent --no-pager
```

## 8. Nginx 反向代理

创建 `/etc/nginx/sites-available/jushi.conf`：
...
```bash
sudo ln -s /etc/nginx/sites-available/jushi.conf /etc/nginx/sites-enabled/jushi.conf
sudo nginx -t
...
- `502 Bad Gateway`：多半是 `jushi-backend` 或 `jushi-agent` 没启动成功。
