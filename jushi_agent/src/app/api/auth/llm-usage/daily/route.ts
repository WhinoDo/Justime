...
export async function GET(request: NextRequest) {
  try {
    ...
    const backendUrl = API_CONFIG.getFullUrl(
      `/auth/llm-usage/daily?days=${encodeURIComponent(days)}&scope=${encodeURIComponent(scope)}`
    )
    ...
    if (!response.ok) {
      return createErrorResponse(
        result.detail || result.message || '获取模型 token 统计失败',
        'FETCH_USAGE_ERROR',
        response.status
      )
    }

    return createSuccessResponse(result.data ?? result, '获取模型 token 统计成功')
  } catch (error) {
    console.error('获取模型 token 统计失败:', error)
