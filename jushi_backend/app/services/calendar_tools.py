...
def _store_suggestion(suggestion: dict):
    """存储日历建议"""
    global _pending_suggestions
    _pending_suggestions.append(suggestion)
    print(f"📅 已存储日历建议: {suggestion.get('event', {}).get('title', 'Unknown')}")
...
def suggest_task_decomposition(
    ...
    Args:
        ...
        subtasks: JSON格式的子任务列表字符串，每个子任务包含: title(标题), duration_hours(预计时长), order(顺序), description(描述)
                  例如: '[{"title":"需求分析","duration_hours":4,"order":1,"description":"收集需求"}]'
        total_days: 总计划天数
...
def create_batch_calendar_events(
    ...
    Args:
        events: JSON格式的事件列表字符串，每个事件包含: title, start_time, end_time, event_type, priority
                例如: '[{"title":"需求分析","start_time":"2026-01-27T09:00:00","end_time":"2026-01-27T13:00:00","event_type":"task","priority":"high"}]'
