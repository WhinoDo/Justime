...
<template>
  ...
  >
    ...
    <div class="relative bg-white border border-gray-200 shadow-xl rounded-xl max-w-[560px] w-full mx-4">
      <div class="p-6 border-b border-gray-200">
        <h3 class="text-lg font-semibold text-gray-900">{{ editingKey ? '编辑 API Key' : '新增 API Key' }}</h3>
      </div>
      ...
      <div class="p-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div class="sm:col-span-2 space-y-1">
          <p class="text-sm text-gray-600">API Key {{ editingKey ? '(留空表示不修改)' : '' }}</p>
          <input
