...
<template>
  <div class="space-y-8">
    ...
    <div class="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
      <!-- Total Users -->
      <div class="p-6 rounded-2xl bg-white border border-gray-200 shadow-sm transition-all hover:shadow-md hover:border-purple-200">
      ...
      <div class="p-6 rounded-2xl bg-white border border-gray-200 shadow-sm transition-all hover:shadow-md hover:border-purple-200">
        ...
        <div>
          ...
          <div class="flex items-center text-xs mt-1">
            <ArrowUpRight class="h-4 w-4 text-green-500 mr-1" />
            <span class="text-green-600">+4.3%</span>
      ...
      <div class="p-6 rounded-2xl bg-white border border-gray-200 shadow-sm transition-all hover:shadow-md hover:border-purple-200">
        <div class="flex flex-row items-center justify-between pb-2">
          <p class="text-sm font-medium text-gray-500">总对话数</p>
          <MessageSquare class="h-4 w-4 text-gray-400" />
        </div>
        <div>
          <div class="text-2xl font-bold text-gray-900">{{ stats?.total_conversations?.toLocaleString() || 0 }}</div>
          <div class="flex items-center text-xs mt-1">
            <ArrowUpRight class="h-4 w-4 text-green-500 mr-1" />
            <span class="text-green-600">+28.4%</span>
      ...
      <div class="p-6 rounded-2xl bg-white border border-gray-200 shadow-sm transition-all hover:shadow-md hover:border-purple-200">
        <div class="flex flex-row items-center justify-between pb-2">
          <p class="text-sm font-medium text-gray-500">Token 消耗</p>
        ...
        <div>
          ...
          <div class="flex items-center text-xs mt-1">
            <ArrowDownRight class="h-4 w-4 text-red-400 mr-1" />
    ...
    <div class="grid grid-cols-1 xl:grid-cols-5 gap-4">
      <!-- Token Trend Chart -->
      <div class="xl:col-span-3 rounded-2xl border border-gray-200 bg-white shadow-sm p-4 lg:p-6">
        <div class="mb-4">
          <h3 class="text-gray-900 text-lg font-semibold">近 7 天 Token 消耗趋势</h3>
          <p class="text-gray-400 text-xs mt-1">基于当前总量推演的日级趋势，用于可视化预览</p>
        </div>
        <div class="h-[280px]">
          ...
          <div class="h-full flex items-end gap-2 px-4">
            ...
            >
              <div
                class="w-full bg-purple-400/70 rounded-t transition-all hover:bg-purple-500"
                :style="{ height: `${(point.tokens / maxTokens) * 240}px` }"
              ></div>
              <span class="text-xs text-gray-500 mt-2">{{ point.date }}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <!-- User Growth Chart -->
      <div class="xl:col-span-2 rounded-2xl border border-gray-200 bg-white shadow-sm p-4 lg:p-6">
        <div class="mb-4">
          <h3 class="text-gray-900 text-lg font-semibold">用户增长与活跃</h3>
          <p class="text-gray-400 text-xs mt-1">近 7 天新增用户与活跃用户对比</p>
        </div>
        <div class="h-[280px]">
          ...
          <div class="h-full flex items-end gap-4 px-2">
            ...
            >
              <div class="flex gap-1 items-end h-[240px]">
                <div
                  class="w-3 bg-purple-400/70 rounded-t transition-all hover:bg-purple-500"
                  :style="{ height: `${(point.newUsers / maxUsers) * 200}px` }"
                ></div>
                <div
                  class="w-3 bg-violet-300/70 rounded-t transition-all hover:bg-violet-400"
    ...
    <!-- System Status -->
    <div class="rounded-2xl border border-gray-200 bg-white shadow-sm p-4 lg:p-6">
      <h3 class="text-gray-900 text-lg font-semibold">系统状态概览</h3>
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
        <div class="flex flex-col space-y-2 p-4 bg-gray-50 border border-gray-200 rounded-lg">
          <span class="text-sm font-medium text-gray-500">后端版本</span>
          <span class="text-lg font-bold text-gray-900">{{ stats?.version || 'Unknown' }}</span>
        </div>
        <div class="flex flex-col space-y-2 p-4 bg-gray-50 border border-gray-200 rounded-lg">
          <span class="text-sm font-medium text-gray-500">数据库连接</span>
          <span class="flex items-center text-lg text-green-600 font-semibold">
            <div class="w-2 h-2 rounded-full bg-green-500 mr-2"></div>
            正常
          </span>
        </div>
        <div class="flex flex-col space-y-2 p-4 bg-gray-50 border border-gray-200 rounded-lg">
          <span class="text-sm font-medium text-gray-500">LLM 服务</span>
          <span class="flex items-center text-lg text-green-600 font-semibold">
            <div class="w-2 h-2 rounded-full bg-green-500 mr-2"></div>
            运行中
          </span>
