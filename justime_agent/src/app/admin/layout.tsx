...
export default function AdminLayout({
        ...
        return (
            <JustimePageShell fullHeight variant={isDesktop ? 'desktop' : 'immersive'} blur={isDesktop ? 'none' : 'lg'} opacity={isDesktop ? 0 : 0.6} contentClassName="flex items-center justify-center">
                <div className="relative z-10 text-center space-y-4">
                    ...
                    <Link href="/">
                        <Button variant="outline" className="border-border bg-muted/50 text-foreground hover:bg-accent/50 hover:text-foreground">返回首页</Button>
                    </Link>
                </div>
            </JustimePageShell>
        )
    ...
    return (
        <div className="min-h-screen relative overflow-hidden font-sans flex bg-background">
            {/* Sidebar */}
            <aside
                className={`fixed inset-y-0 left-0 z-50 w-64 bg-muted/50 backdrop-blur-xl border-r border-border transform transition-transform duration-200 ease-in-out ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'
            ...
            >
                <div className="h-full flex flex-col">
                                ...
                                <Link key={item.href} href={item.href}>
                                    <div
                                        className={`flex items-center gap-3 px-4 py-3 rounded-lg transition-colors ${isActive
                                                ? 'bg-accent/50 text-foreground shadow-inner'
                    ...
                    <div className="px-4 py-2 border-t border-border">
                        <Link href="/dashboard">
                            <div className="flex items-center gap-3 px-4 py-3 rounded-lg transition-colors text-muted-foreground hover:bg-accent/50 hover:text-foreground cursor-pointer">
                    ...
                    <div className="p-4 border-t border-border">
                        <div className="flex items-center gap-3 px-4 py-3">
                            <div className="w-8 h-8 rounded-full bg-accent/50 flex items-center justify-center text-foreground font-bold">
                                {user.displayName?.[0]?.toUpperCase() || 'A'}
                            </div>
                            <div className="flex-1 min-w-0">
            ...
            <main className="relative z-10 flex-1 flex flex-col min-w-0 overflow-hidden">
                {/* Mobile Header */}
                <div className="lg:hidden h-16 bg-muted/50 backdrop-blur-xl border-b border-border flex items-center px-4 relative z-20">
                    <button
                        onClick={() => setIsSidebarOpen(true)}
                    ...
                    <span className="ml-4 text-lg font-semibold text-foreground">管理后台</span>
                </div>
